...
function legacyKeys(key: StorageKey): string[] {
  ...
  const legacyMap: Record<string, string[]> = {
    ...
    exercises_cache: ["pulse_exercises_cache", "pulse_exercises_cache_v2", "relift_exercises_cache"],
  ...
  migrateLegacy(): void {
    try {
      ...
      for (const k of keysToMigrate) {
        const legacyKey = legacyKeys(k).find(lk => lk !== makeKey(k) && localStorage.getItem(lk) !== null);
        if (legacyKey) {
