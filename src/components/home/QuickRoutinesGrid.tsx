...
import fullBodyImg from "@/assets/images/full_body_routine_1784768080165.jpg";
  ...
  return (
    <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={3}>
      <div className="mb-3 flex items-center justify-between">
        ...
        <Link to="/builder" className="flex items-center gap-1 text-xs font-black text-primary hover:text-primary-hover uppercase tracking-wider cursor-pointer">
        ...
        {routines.map((routine, idx) => (
          ...
          >
            <div className="relative w-full h-28 mb-2 rounded-xl overflow-hidden border border-white/10 group-hover:border-primary/40 transition-all shadow-md">
              <img src={routine.image} alt={routine.name} referrerPolicy="no-referrer" className="w-full h-full object-cover object-center group-hover:scale-105 transition-transform duration-500" />
              <div className="absolute inset-0 bg-gradient-to-t from-bg-surface via-transparent to-black/30" />
              <div className="absolute top-2 left-2 z-10">
                <span className={cn("text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md border backdrop-blur-md shadow-sm", routine.badgeStyle)}>{routine.badge}</span>
              </div>
              <div className="absolute top-2 right-2 z-10">
                <span className="text-[9px] font-black text-white bg-black/70 backdrop-blur-md px-2 py-0.5 rounded-md border border-white/10 flex items-center gap-1">
                  <Clock size={10} className="text-primary" />{routine.duration}
                </span>
              </div>
            </div>

            <div className="relative z-10 mb-2">
              <h3 className="text-sm font-black italic tracking-tight text-text-primary group-hover:text-primary transition-colors">{routine.name}</h3>
              <p className="text-[10px] text-text-muted font-bold mt-0.5 line-clamp-1">{routine.subtitle}</p>
            </div>
