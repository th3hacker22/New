...
import { 
  Calendar, Clock, Dumbbell, Trash2, Search, Filter, 
  ChevronDown, ChevronUp, BarChart2, TrendingUp, Sparkles, 
  CheckCircle2, AlertCircle, Info, Flame, Trophy 
} from "lucide-react";
  ...
  const filteredSessions = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(s => {
        ...
        const exerciseMatch = s.exercises.some(ex => 
      ...
      return (
        <div className="bg-bg-surface border border-border/85 rounded-xl p-3 shadow-xl backdrop-blur-md">
          ...
          <p className="text-xs font-bold text-primary mt-1">
            {isAr ? "الحجم الإجمالي:" : "Total Volume:"} <span className="font-mono font-black text-sm">{data.volume.toLocaleString()} kg</span>
          </p>
        </div>
      );
    }
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-center w-full">
          ...
          <div className="relative flex-1 w-full">
            <input
              ...
              placeholder={isAr ? "ابحث بالاسم أو التمرين أو العضلة..." : "Search workouts or exercises..."}
            ...
            />
            <Search className={cn("absolute top-1/2 -translate-y-1/2 w-4 h-4 text-text-muted/60", isAr ? "right-3" : "left-3")} />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery("")}
                className={cn("absolute top-1/2 -translate-y-1/2 text-text-muted hover:text-text-primary font-bold text-xs p-1", isAr ? "left-3" : "right-3")}
              >
                ×
              </button>
            )}
      ...
      <div className="grid grid-cols-3 gap-3 bg-bg-surface-hover/10 rounded-2xl p-4 border border-border/30">
        ...
        <div className="text-center p-1.5 border-x border-border/40">
          ...
          <p className="text-base font-black text-text-primary tabular-nums">
            {historyStats.totalVolume.toLocaleString()} <span className="text-[9px] text-text-muted">{isAr ? "كجم" : "kg"}</span>
          </p>
        </div>
        <div className="text-center p-1.5">
          <p className="text-[9px] text-text-muted font-black uppercase tracking-widest mb-1">
          ...
          <p className="text-base font-black text-text-primary tabular-nums">
            {historyStats.totalDurationMins} <span className="text-[9px] text-text-muted">{isAr ? "دق" : "min"}</span>
          </p>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              ...
              <p className="text-[10px] text-text-muted font-bold">
                {isAr ? "مراقبة زيادة القوة والأوزان الإجمالية بمرور الوقت" : "Track overall strength load gains over time"}
              </p>
            </div>
            ...
            {chartData.length >= 2 ? (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData} margin={{ top: 5, right: 15, left: -10, bottom: 5 }}>
                  ...
                  <Line 
                    ...
                    strokeWidth={2.5}
                    dot={{ r: 3.5, stroke: "#111", strokeWidth: 1.5, fill: "var(--c-primary, #ccff00)" }}
                    activeDot={{ r: 5, stroke: "#111", strokeWidth: 2, fill: "var(--c-primary, #ccff00)" }}
                  />
                </LineChart>
              </ResponsiveContainer>
            ) : (
              <div className="h-full flex flex-col items-center justify-center text-center space-y-2 bg-bg-surface-hover/10 rounded-xl border border-border/20 p-4">
                ...
                <p className="text-[10px] font-black text-text-muted uppercase tracking-wider">
                  {isAr ? "تحتاج إلى جلستي تمرين على الأقل لعرض المخطط" : "At least 2 workouts needed for progression chart"}
                </p>
        ...
        {isLoadingCompleted ? (
          ...
          <div className="space-y-3.5">
            {filteredSessions.map((session, sIdx) => {
              ...
              const completedSetsCount = session.exercises.reduce((acc, ex) => acc + ex.sets.length, 0);
              ...
              return (
                <motion.div
                  ...
                  className={cn(
                    ...
                    isExpanded ? "border-primary/20 shadow-lg shadow-primary/5" : "border-border/40"
                  )}
                >
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <div className={cn(
                        ...
                        isExpanded ? "bg-primary/20 border-primary/30 text-primary" : "bg-bg-surface-hover/85 border-border/40 text-text-muted"
                      )}>
                        ...
                        <div className="p-4 space-y-4">
                          ...
                          <div className="grid grid-cols-2 gap-2 text-center sm:hidden">
                            <div className="bg-bg-surface p-2 rounded-xl border border-border/30">
                              <span className="text-[9px] text-text-muted font-bold block">{isAr ? "الحجم الإجمالي" : "Total Volume"}</span>
                              <span className="text-xs font-black text-primary font-mono">{sessionVolume.toLocaleString()} kg</span>
                            </div>
                            <div className="bg-bg-surface p-2 rounded-xl border border-border/30">
                              <span className="text-[9px] text-text-muted font-bold block">{isAr ? "عدد التمارين" : "Exercises"}</span>
                              <span className="text-xs font-black text-text-primary font-mono">{session.exercises.length}</span>
                            </div>
                          </div>
                          ...
                          <div className="space-y-3">
                            <span className="text-[9px] font-black text-text-muted uppercase tracking-wider block">
                              🏋️ {isAr ? "تفاصيل الأداء والمجموعات:" : "Exercises & Completed Sets:"}
                            </span>
                          ...
                          <div className="flex items-center justify-between border-t border-border/30 pt-3">
                            <div className="text-[9px] text-text-muted font-semibold flex items-center gap-1">
                              ...
                              {isAr ? "تم الحفظ محلياً وبالمزامنة السحابية" : "Saved to local cache & synchronized with cloud"}
                            </div>
