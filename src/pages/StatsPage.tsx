...
export default function StatsPage() {
  ...
  useEffect(() => {
    async function loadData() {
      if (isDemoMode) {
        ...
        setPersonalRecords([
          { exerciseId: "demo-bench", exerciseName: isAr ? "بنش بريس بالبار" : "Barbell Bench Press", maxWeight: 100, date: new Date().toISOString() },
          { exerciseId: "demo-squat", exerciseName: isAr ? "اسكوات بالبار" : "Barbell Squat", maxWeight: 130, date: new Date().toISOString() },
        ]);
        const mockVol = Array.from({ length: 8 }, (_, i) => ({ week: `W${i+1}`, volume: 18000 + i*1500 }));
        setWeeklyVolume(mockVol);
        setWeeklyTonnage(mockVol.map(v => ({ week: v.week, tonnage: v.volume })));
        setMuscleGroupStats([{ muscle: "Chest", volume: 28400 }, { muscle: "Back", volume: 24500 }, { muscle: "Legs", volume: 32100 }]);
        setWeeklySetVolume([{ muscle: "Chest", sets: 18 }, { muscle: "Back", sets: 15 }]);
        setWorkoutDensity(Array.from({ length: 84 }, (_, i) => ({ date: new Date(Date.now() - i*86400000).toISOString().split('T')[0], count: i % 3 === 0 ? 1 : 0 })));
        const now = new Date();
        setSessionsList([{ id: "demo", name: "Push Day", date: now.toISOString(), duration: 3600, exercises: [{ exerciseId: "bench", exerciseName: "Bench", muscleGroup: "Chest", sets: [{ weight: 80, reps: 10, completed: true }] }] }]);
        ...
        setMuscleRecovery({ Chest: { percent: 92, hoursLeft: 4.2 }, Back: { percent: 45, hoursLeft: 26.5 }, Legs: { percent: 12, hoursLeft: 42.1 }, Shoulders: { percent: 78, hoursLeft: 10.5 }, Arms: { percent: 100, hoursLeft: 0 }, Core: { percent: 60, hoursLeft: 19.2 } });
        setWorkedThisWeek({ Chest: 100, Back: 100, Legs: 100, Shoulders: 100, Arms: 100, Core: 100 });
        setE1rms([{ exerciseName: "Bench Press", e1rm: 118 }, { exerciseName: "Squat", e1rm: 151 }]);
        return;
      }

      const [streakData, statsData, prsData, volumeData, tonnageData, muscleData, densityData, setVolumeData, allSessions] = await Promise.all([
        getWorkoutStreak(), getTotalStats(), getPersonalRecords(), getWeeklyVolume(10), getWeeklyTonnage(10), getMuscleGroupStats(exercises), getWorkoutDensity(), getWeeklySetVolume(exercises), db.workoutSessions.where("completed").equals(true).toArray(),
      ]);
      ...
      const calcTrend = (data: any[], mapper: (week: string) => number) => data.map((v) => ({ value: mapper(v.week) }));
      setWorkoutSparkline(calcTrend(volumeData, (week) => allSessions.filter((s: any) => { const d = new Date(s.date); const start = new Date(d.getFullYear(),0,1); const days = Math.floor((d.getTime() - start.getTime())/86400000); return `W${Math.ceil((days + start.getDay()+1)/7)}` === week; }).length));
      setVolumeSparkline(volumeData.map((v) => ({ value: v.volume || 0 })));
      setDurationSparkline(volumeData.map((v) => { const total = allSessions.filter((s: any) => { const d = new Date(s.date); const start = new Date(d.getFullYear(),0,1); const days = Math.floor((d.getTime() - start.getTime())/86400000); return `W${Math.ceil((days + start.getDay()+1)/7)}` === v.week; }).reduce((acc: number, s: any) => acc + (s.duration||0),0); return { value: Math.round(total/60) }; }));
      ...
      for (const muscle of MUSCLE_GROUPS) {
        const relevant = allSessions.filter((s: any) => s.exercises.some((ex: any) => { const def = exercises.find((e) => String(e.id) === String(ex.exerciseId)); const cat = getCategory(ex) || (def ? getCategory(def) : ""); return cat === muscle; }));
        const latest = relevant.sort((a: any, b: any) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];
        if (!latest) recoveryMap[muscle] = { percent: 100, hoursLeft: 0 };
        else { const elapsed = (nowTime - new Date(latest.date).getTime())/3600000; const left = Math.max(0,48-elapsed); recoveryMap[muscle] = { percent: Math.min(100, Math.round((elapsed/48)*100)), hoursLeft: Math.round(left*10)/10 }; }
        weeklyWorked[muscle] = relevant.filter((s: any) => new Date(s.date).getTime() >= oneWeekAgo).length > 0 ? 100 : 0;
      }
      setMuscleRecovery(recoveryMap);
      setWorkedThisWeek(weeklyWorked);

      const calc1rms = await Promise.all(prsData.slice(0,4).map(async (pr: any) => { const data = await getEstimated1RM(pr.exerciseId); const best = data.length > 0 ? Math.max(...data.map((d) => d.e1rm)) : 0; return { exerciseName: pr.exerciseName, e1rm: best }; }));
  ...
  async function loadMeasurementsAndPhotos() {
    if (isDemoMode) {
      const mockM = [{ id: "m1", date: new Date().toISOString(), weight: 80.2, bodyFat: 14.7, waist: 84.8, chest: 105.5, createdAt: "", updatedAt: "" }];
      setMeasurements(mockM as any);
      const mockP = [{ id: "p1", date: new Date().toISOString(), type: "front" as const, imageBlob: new Blob(), url: "https://images.unsplash.com/photo-1517838277536-f5f99be501cd?q=80&w=600&auto=format&fit=crop", createdAt: "" }];
      setPhotos(mockP as any);
      setBeforePhotoId("p1"); setAfterPhotoId("p1");
      return;
    }
    const [mData, pData] = await Promise.all([db.bodyMeasurements.orderBy("date").reverse().toArray(), db.progressPhotos.orderBy("date").reverse().toArray()]);
    setMeasurements(mData);
    const withUrls = pData.map((p) => ({ ...p, url: URL.createObjectURL(p.imageBlob) }));
    setPhotos(withUrls);
    if (withUrls.length >= 2) { setBeforePhotoId(withUrls[withUrls.length-1].id||""); setAfterPhotoId(withUrls[0].id||""); }
  }
  ...
  async function handlePhotoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const photo: ProgressPhoto = { id: uid(), date: new Date().toISOString(), type: "front", imageBlob: file, createdAt: new Date().toISOString() };
  ...
  return (
    <div className="space-y-6 pb-20 pt-2 animate-fade-in" dir={isAr ? "rtl" : "ltr"}>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-border/40 pb-3">
        <div className="flex items-center gap-1.5 p-1 bg-bg-surface/80 backdrop-blur-md rounded-2xl border border-border/50 overflow-x-auto no-scrollbar shadow-inner">
          {[
            ...
            return (
              <button key={tab.id} onClick={() => { setActiveTab(tab.id as any); if (typeof navigator !== "undefined" && navigator.vibrate) navigator.vibrate(10); }} className={cn("relative flex items-center gap-2 px-3.5 py-1.5 rounded-xl text-xs font-black transition-all cursor-pointer shrink-0 select-none", isActive ? "text-primary-text font-black" : "text-text-muted hover:text-text-primary hover:bg-bg-surface-hover/50")}>
                {isActive && <motion.div layoutId="activeSubTabPill" className="absolute inset-0 bg-primary rounded-xl shadow-[0_0_15px_rgba(204,255,0,0.4)]" transition={{ type: "spring", stiffness: 400, damping: 28 }} />}
                <span className="relative z-10 flex items-center gap-1.5"><Icon size={14} strokeWidth={isActive ? 2.5 : 2} /><span>{tab.label}</span></span>
              </button>
            );
          })}
        </div>
      ...
      <AnimatePresence mode="wait">
        {activeTab === "overview" && (
          <motion.div key="overview-tab" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }}>
            <OverviewTab
              streak={streak}
        ...
        {activeTab === "exercises" && (
          <motion.div key="exercises-tab" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}><ExercisesTab e1rms={e1rms} /></motion.div>
        )}
        {activeTab === "measurements" && (
          <motion.div key="measurements-tab" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
            <MeasurementsTab measurements={measurements} isAr={isAr} onAdd={handleAddMeasurement} showAdd={showAddMeasurement} onShowAdd={setShowAddMeasurement} />
          </motion.div>
        )}
        {activeTab === "photos" && (
          <motion.div key="photos-tab" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
            <PhotosTab photos={photos} beforePhotoId={beforePhotoId} afterPhotoId={afterPhotoId} sliderPos={sliderPos} isAr={isAr} onBeforeChange={setBeforePhotoId} onAfterChange={setAfterPhotoId} onSliderChange={setSliderPos} onUpload={handlePhotoUpload} onDelete={deletePhoto} />
          </motion.div>
        )}
        {activeTab === "history" && (
          <motion.div key="history-tab" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}><HistoryTab isAr={isAr} /></motion.div>
        )}
      </AnimatePresence>
    </div>
  );
